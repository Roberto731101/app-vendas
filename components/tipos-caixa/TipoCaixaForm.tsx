...
export function TipoCaixaForm({
  nome, pesoPadrao, observacao, ativo,
  setNome, setPesoPadrao, setObservacao, setAtivo,
  ...
  return (
    <section className="space-y-6 rounded-2xl bg-white p-6 shadow-sm">
      <h3 className="text-sm font-bold uppercase tracking-wider text-[#063f81]">
        {editando ? 'Editar Tipo de Caixa' : 'Novo Tipo de Caixa'}
      </h3>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="Ex: Caixa Plástica"
            className="w-full rounded-xl border-none bg-slate-100 px-4 py-3 text-sm outline-none"
          />
        </div>

        <div className="space-y-1.5">
          <label className="ml-1 text-xs font-bold text-slate-500">Peso Padrão (kg)</label>
          <input
            type="number"
            step="0.01"
            value={pesoPadrao}
